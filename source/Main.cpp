...
public:
    ...
    static inline bool wasPointing;
    static inline bool wasCrouching;
    static inline bool ignoreRotation;
    static inline bool forceRealMoveAnim;
    static inline RwV3d lastLockOnPos;
    static inline int timeLockOn;
    static inline CRGBA lastLockOnColor;
    static inline CPed* thirdPersonMouseTarget;
    static inline short previousCamMode;
    ...
    static inline bool weaponInfoSet;
    static inline class CWeaponInfo weaponInfo[WEAPONTYPE_HELICANNON + 1];
#endif
...
public:
    ClassicAxis() {
        ...
        plugin::Events::gameProcessEvent += [] {
            if (!weaponInfoSet) {
                memcpy(&weaponInfo, aWeaponInfo, sizeof(CWeaponInfo) * WEAPONTYPE_HELICANNON + 1);
                weaponInfoSet = true;
            }
        };
#endif
    ...
    // No fight cam
        ...
        auto playerShootingDirection = [](int, int) {
            ...
            if (isAiming && !CWorld::Players[CWorld::PlayerInFocus].m_pPed->m_bHasLockOnTarget)
                moveType = TYPE_STRAFE;
        ...
        onFiring.before += [](CWeapon*, CEntity*, CVector*) {
            if (savedCamMode == -1) {
                savedCamMode = TheCamera.m_asCams[TheCamera.m_nActiveCam].m_nCamMode;
                TheCamera.m_asCams[TheCamera.m_nActiveCam].m_nCamMode = MODE_FOLLOW_PED;
            }
        };
        onFiring.after += [](CWeapon*, CEntity*, CVector*) {
            if (savedCamMode != -1) {
                TheCamera.m_asCams[TheCamera.m_nActiveCam].m_nCamMode = savedCamMode;
                savedCamMode = -1;
            }
        };
        ...
        auto camControl = [](int, int) {
            ...
            TheCamera.m_f3rdPersonCHairMultX = settings.cameraCrosshairMultX;
            TheCamera.m_f3rdPersonCHairMultY = settings.cameraCrosshairMultY;
...
#ifdef GTA3
            AdjustWeaponInfoForCrouch(playa);
#endif
            ProcessPlayerPedControl(playa);
            Find3rdPersonMouseTarget(playa);
        };
        ...
        plugin::Events::drawHudEvent += [] {
            ...
            if (settings.showTriangleForMouseRecruit)
        ...
        onDucking += [](CPed* ped, int, __int16) {
            ...
            if (!info->m_bCrouchFire && !IsTypeMelee(ped)) {
                ped->bCrouchWhenShooting = false;
        ...
        onFindingNewLockOnTarget += [](CPlayerPed* ped) {
            ...
            if ((!ped->bIsDucking || info->m_bCrouchFire)) {
        ...
        onSet1stPersonPlayerCamMode += [](CCam* cam, int, int, int) {
            if (TheCamera.m_asCams[TheCamera.m_nActiveCam].m_nCamMode == MODE_FOLLOW_PED) {
                ...
                RotatePlayer(playa, angle, false);
            }
        };
...
#else
                CMatrix& mat = *static_cast<CMatrix*>(cam->m_pCamTargetEntity);
#endif
    ...
    static bool IsAbleToAim(CPed* ped) {
...
#ifdef GTAVC
                    && (!ped->bIsDucking || info->m_bCrouchFire)
#endif
                    ;
            default:
                return false;
    ...
    static bool IsType1stPerson(CPed* ped) {
        const eWeaponType weaponType = ped->m_aWeapons[ped->m_nCurrentWeapon].m_eWeaponType;
        CWeaponInfo* info = CWeaponInfo::GetWeaponInfo(weaponType);
    ...
    static bool IsWeaponPossiblyCompatible(CPed* ped) {
        const eWeaponType weaponType = ped->m_aWeapons[ped->m_nCurrentWeapon].m_eWeaponType;
        CWeaponInfo* info = CWeaponInfo::GetWeaponInfo(weaponType);
...
#ifdef GTA3
    static bool CanDuckWithThisWeapon(eWeaponType weapon) {
        switch (weapon) {
    ...
    static bool IsTypeMelee(CPed* ped) {
        const eWeaponType weaponType = ped->m_aWeapons[ped->m_nCurrentWeapon].m_eWeaponType;
    ...
    static bool IsTypeTwoHanded(CPed* ped) {
        const eWeaponType weaponType = ped->m_aWeapons[ped->m_nCurrentWeapon].m_eWeaponType;
        CWeaponInfo* info = CWeaponInfo::GetWeaponInfo(weaponType);
            ...
            if (crosshair) {
                if (isAiming) {
                    if (!playa->m_bInVehicle && (mode == MODE_FOLLOW_PED || mode == MODE_AIMWEAPON) && !pad->DisablePlayerControls) {
                        if (!playa->m_bHasLockOnTarget && IsWeaponPossiblyCompatible(playa))
    ...
    static void DrawTriangleForMouseRecruitPed() {
        if (TheCamera.m_nTransitionState != 0)
            return;
    ...
    static void ClearWeaponTarget(CPlayerPed* ped) {
    ...
    static inline std::string controlKeysStrings[62] = {
        "ESC",
        "F1",
        "F2",
        "F3",
    ...
    static int32_t StringToKey(std::string str) {
        int32_t key = str[0];
    ...
    static bool GetKeyDown(int32_t key, bool old = false) {
        CPad* pad = CPad::GetPad(0);
    ...
    static void ProcessPlayerPedControl(CPlayerPed* playa) {
        ...
        if (IsType1stPerson(playa))
            return;

        if (!pad->DisablePlayerControls && IsAbleToAim(playa) &&
#ifdef GTA3
        (!playa->bIsDucking || CanDuckWithThisWeapon(weaponType)) &&
#endif
            pad->GetTarget() && (TheCamera.GetLookDirection() != 0) && IsWeaponPossiblyCompatible(playa) && (mode == MODE_FOLLOW_PED || mode == MODE_AIMWEAPON)
            ...
            if (mode != MODE_AIMWEAPON && playa->IsPedInControl() && TheCamera.m_nTransitionState == 0) {
                ...
                previousHorAngle = cam.m_fHorizontalAngle;
                previousVerAngle = cam.m_fVerticalAngle;
            ...
            if (!info->m_bCanAimWithArm || playa->bIsDucking) {
            ...
            if (point) {
                if (playa->m_ePedState != PEDSTATE_AIMGUN) {
                    ...
                    if (playa->bCrouchWhenShooting && playa->bIsDucking) {
            ...
            if (playa->bIsDucking && !wasCrouching) {
        ...
        if (!isAiming) {
            ...
            if (wasCrouching) {
                if (currentWeapon.m_eWeaponState != WEAPONSTATE_OUT_OF_AMMO && currentWeapon.m_eWeaponState != WEAPONSTATE_RELOADING &&
                    IsAbleToAim(playa)) {
                    playa->bCrouchWhenShooting = true;
#ifdef GTA3
        ...
        if (settings.crouchKey != "NULL" && !pad->DisablePlayerControls && (mode == MODE_FOLLOW_PED || mode == MODE_AIMWEAPON) && IsAbleToAim(playa)) {
            ...
            if (!playa->bIsDucking && duckJustDown && playa->m_eMoveState != PEDMOVE_SPRINT) {
                playa->bCrouchWhenShooting = true;
                SetDuck(playa);
            }
            else if (playa->bIsDucking && (duckJustDown || playa->m_eMoveState == PEDMOVE_SPRINT || pad->GetSprint() || pad->JumpJustDown() || pad->ExitVehicleJustDown() || (pad->GetWeapon() && !CanDuckWithThisWeapon(weaponType)))) {
                ClearDuck(playa);
            }

            if (playa->bIsDucking && !isAiming && (!pad->GetWeapon() || playa->m_ePedState != PEDSTATE_ATTACK || !playa->bIsAttacking)) {
                ...
                playa->ClearPointGunAt();
                playa->bIsDucking = true;
            }

            if (playa->bIsDucking && currentWeapon.HasWeaponAmmoToBeUsed() && playa->m_ePedState == PEDSTATE_ATTACK && currentWeapon.m_nAmmoInClip <= 0) {
    ...
    static void Find3rdPersonMouseTarget(CPlayerPed* ped) {
    ...
    static void SetDuck(CPlayerPed* ped) {
        if (ped->bIsDucking || CTimer::m_snTimeInMilliseconds <= ped->m_nDuckTimer) {
    ...
    static void ClearDuck(CPlayerPed* ped) {
    ...
    static void AdjustWeaponInfoForCrouch(CPlayerPed* ped) {
        if (!ped->bIsDucking)
            return;
    ...
    static void ResetWeaponInfo(CPlayerPed* ped) {
        if (!wantsToResetWeaponInfo)
            return;

        memcpy(&aWeaponInfo, weaponInfo, sizeof(CWeaponInfo) * WEAPONTYPE_HELICANNON);
